...
export function SectionHeading({ title, subtitle, className, ...props }: SectionHeadingProps) {
  return (
    <div className={cn("mb-12 flex flex-col items-center justify-center text-center", className)}>
      <h2 
        className="text-3xl font-bold tracking-tight sm:text-4xl md:text-5xl bg-clip-text text-transparent bg-gradient-to-r from-zinc-100 to-zinc-500"
        {...props}
      >
        {title}
      </h2>
      ...
      {subtitle && (
        <p className="mt-4 max-w-[700px] text-lg text-zinc-400">
          {subtitle}
        </p>
      )}
    </div>
